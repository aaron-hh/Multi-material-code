...
int      AmrLevelAdv::NUM_STATE       = 15;  // One variable in the state
    ...
    for(int k = lo.z; k <= hi.z; k++)
    {
      for(int j = lo.y; j <= hi.y; j++)
      {
        for(int i = lo.x; i <= hi.x+2; i++)
        {
          ...
          // double alpha_b = 1.0 - arr(i,j,k,0); // volume fraction of material b
          // double rho_a = arr(i,j,k,1) / alpha_a; // density for material a
          // double rho_b = arr(i,j,k,2) / alpha_b; // density for material b
          // double density = alpha_a * rho_a + alpha_b * rho_b; // mixture density
          
          // double vx = arr(i,j,k,5) / density; // normal velocity vx
          // double vy = arr(i,j,k,7) / density; // normal velocity vy
          
          // double eps_a = arr(i,j,k,3) / (alpha_a * rho_a);
          // double eps_b = arr(i,j,k,4) / (alpha_b * rho_b);

          // double p_a = EOS->epstopressure(rho_a, eps_a, pinf_a, gamma_a);
          // double p_b = EOS->epstopressure(rho_b, eps_b, pinf_b, gamma_b);

          // double energy = arr(i,j,k,6) / density;//*EOS->compute_energy(alpha_a, rho_a, eps_a, rho_b, eps_b, vx, vy);
          // //double pressure = EOS->compute_pressure(alpha_a, rho_a, gamma_a, pinf_a, rho_b, gamma_b, pinf_b, energy, vx, vy);
          // double pressure = alpha_a * p_a + alpha_b * p_b;

          // const Real x = probLoX + (double(i)+0.5) * dX;
          // const Real y = probLoY + (double(j)+0.5) * dY;
          // // output<<x<<" "<<y<<" "<<arr(i,j,k,0)<<" "<<arr(i,j,k,1)<<" "<<arr(i,j,k,2)<<" "<<arr(i,j,k,3)<<" "<<arr(i,j,k,4)<<" "<<arr(i,j,k,5)<<" "<<arr(i,j,k,6)<<" "<<arr(i,j,k,7)<<" "<<density<<" "<<vx<<" "<<pressure<<" "<<vy<<std::endl;
          // // //output<<x<<" "<<y<<" "<<alpha_a<<" "<<density<<" "<<vx<<" "<<energy<<" "<<eps_a<<" "<<eps_b<<" "<<vy<<std::endl;
          // output<<x<<" "<<y<<" "<<arr(i,j,k,1)<<" "<<arr(i,j,k,2)<<" "<<alpha_a<<" "<<density<<" "<<vx<<" "<<pressure<<" "<<energy<<" "<<vy<<std::endl;
        }
        // output<<std::endl;
      }
    }  
...
// 2020H: setting up the boundary conditions
// // Boundary condition for 1D test 3
...
//   BCRec bc(lo_bc, hi_bc);
  ...
  // //Boundary condition for Test 4
  // //Boundary condition for scalar variables
  ...
  //   if(i==0)
  //   {
  //     lo_bc[i] = BCType::reflect_even;
  //     hi_bc[i] = BCType::foextrap;
  //   }

  //   else if(i==1)
  //   {
  //     lo_bc[i] = hi_bc[i]= BCType::reflect_even;
  //   }
  // }
  // // Object for storing all the boundary conditions
  // BCRec bc(lo_bc, hi_bc);

  // // Boundary condition for v_x
  // int lo_bc2[amrex::SpaceDim];
  // int hi_bc2[amrex::SpaceDim];
  // for (int i = 0; i < amrex::SpaceDim; ++i) 
  // {
  //   if(i==0)
  //   {
  //     lo_bc2[i] = BCType::reflect_odd;
  //     hi_bc2[i] = BCType::foextrap;
  //   }

  //   else if(i==1)
  //   {
  //     lo_bc2[i] = BCType::reflect_even;
  //     hi_bc2[i] = BCType::reflect_even;
  //   }
  // }
  // // Object for storing all the boundary conditions
  // BCRec bc2(lo_bc2, hi_bc2);

  // // Boundary condition for v_y
  // int lo_bc3[amrex::SpaceDim];
  // int hi_bc3[amrex::SpaceDim];
  // for (int i = 0; i < amrex::SpaceDim; ++i) 
  // {
  //   if(i==0)
  //   {
  //     lo_bc3[i] = BCType::reflect_even;
  //     hi_bc3[i] = BCType::foextrap;
  //   }

  //   else if(i==1)
  //   {
  //     lo_bc3[i] = BCType::reflect_odd;
  //     hi_bc3[i] = BCType::reflect_odd;
  //   }
  // }
  // // Object for storing all the boundary conditions
  // BCRec bc3(lo_bc3, hi_bc3);

// // Boundary condition for Test 5
//   // Boundary condition for scalar variables
//   int lo_bc[amrex::SpaceDim];
//   int hi_bc[amrex::SpaceDim];
//   for(int i=0; i< amrex::SpaceDim; i++)
...
//       hi_bc[i] = BCType::reflect_even; // same sign
//     }

//     else if(i==1)
//     {
//       lo_bc[i] = hi_bc[i]= BCType::foextrap;
//     }
//   }
//   // Object for storing all the boundary conditions
//   BCRec bc(lo_bc, hi_bc);
...
//     if(i==0)
//     {
//       lo_bc2[i] = BCType::foextrap;
//       hi_bc2[i] = BCType::reflect_odd; // flip sign
//       // hi_bc2[i] = BCType::ext_dir; // HHN21: vibrating bc 
//     }

//     else if(i==1)
//     {
//       lo_bc2[i] = hi_bc2[i] = BCType::foextrap;
//     }
//   }
//   // Object for storing all the boundary conditions
//   BCRec bc2(lo_bc2, hi_bc2);

// Boundary condition for Test 5 with droplet hitting bottom wall
  // Boundary condition for scalar variables
  int lo_bc[amrex::SpaceDim];
  int hi_bc[amrex::SpaceDim];
  for(int i=0; i< amrex::SpaceDim; i++)
  ...
  //   if(i==0)
  //   {
  //     lo_bc2[i] = hi_bc2[i] = BCType::reflect_odd;
  //   }
  ...
  //   if(i==0)
  //   {
  //     lo_bc3[i] =  hi_bc3[i] = BCType::foextrap;
  //   }

  //   else if(i==1)
  //   {
  //     lo_bc3[i] = hi_bc3[i] = BCType::reflect_odd;
  //   }
  // }
  // // Object for storing all the boundary conditions
  // BCRec bc3(lo_bc3, hi_bc3);
  ...
  // 2020H: Added new conservative variables for cavitation solver using diffuse interface approach
  ...
  desc_lst.setComponent(Phi_Type, 3, "alpha_rho_eps_a", bc, bndryfunc2);
  desc_lst.setComponent(Phi_Type, 4, "alpha_rho_eps_b", bc, bndryfunc2);
  desc_lst.setComponent(Phi_Type, 5, "rho_vx", bc3, bndryfunc2); 
  desc_lst.setComponent(Phi_Type, 6, "rho_energy", bc, bndryfunc2);
  desc_lst.setComponent(Phi_Type, 7, "rho_vy", bc2, bndryfunc2);
  desc_lst.setComponent(Phi_Type, 8, "S_star", bc, bndryfunc2);
  desc_lst.setComponent(Phi_Type, 9, "yS_star", bc, bndryfunc2);
  desc_lst.setComponent(Phi_Type, 10, "rho", bc, bndryfunc2);
  desc_lst.setComponent(Phi_Type, 11, "vy", bc, bndryfunc2);
  desc_lst.setComponent(Phi_Type, 12, "S", bc, bndryfunc2);
  desc_lst.setComponent(Phi_Type, 13, "yS", bc, bndryfunc2);
  desc_lst.setComponent(Phi_Type, 14, "Pressure", bc, bndryfunc2);
}
...
AmrLevelAdv::initData ()
{
  ...
  double alpha_bl = 1 - alpha_al;
  double p_al = p_l; 
  double p_bl = p_l;
  double eps_al = (p_al + gamma_a * pinf_a) / ((gamma_a - 1) * rho_al); // variable 3
  double eps_bl = (p_bl + gamma_b * pinf_b) / ((gamma_b - 1) * rho_bl); // variable 4

  double alpha_br = 1 - alpha_ar;
  double p_ar = p_r; 
  double p_br = p_r;
  double eps_ar = (p_ar + gamma_a * pinf_a) / ((gamma_a - 1) * rho_ar); // primitive variable 3
  double eps_br = (p_br + gamma_b * pinf_b) / ((gamma_b - 1) * rho_br); // primitive variable 4

  // 2020H: defining initial condition in conservative variable
  // Loop over all the patches at this level
  for (MFIter mfi(S_new); mfi.isValid(); ++mfi)
  {
    ...
    for(int k = lo.z; k <= hi.z; k++)
    {
      ...
      // const Real z = probLoZ + (double(k)-1.0) * dZ;
      for(int j = lo.y; j <= hi.y; j++)
      {
        const Real y = probLoY + (double(j)+0.5) * dY;
        // const Real y = probLoY + (double(j)-1.0) * dY;
        for(int i = lo.x; i <= hi.x; i++)
        {
          const Real x = probLoX + (double(i)+0.5) * dX;
          // const Real x = probLoX + (double(i)-1.0) * dX;

          // // Initial condition for test 3
          // if(y<1-x)//(sqrt(pow((x-1),2)+pow((y-1),2))<0.4)
          // {
          //   arr(i,j,k,0) = alpha_al; // alpha_a
          //   arr(i,j,k,1) = alpha_al * rho_al; // alpha_a * rho_a
          //   arr(i,j,k,2) = alpha_bl * rho_bl; // alpha_b * rho_b
          //   arr(i,j,k,3) = alpha_al * rho_al * eps_al; // alpha_a * rho_a * eps_xa
          //   arr(i,j,k,4) = alpha_bl * rho_bl * eps_bl; // alpha_b * rho_b * eps_xb

          //   // rho * v_x
          //   double rho_l = alpha_al * rho_al + alpha_bl * rho_bl;
          //   arr(i,j,k,5) = rho_l * vx_l;

          //   // rho * energy
          //   double y_al = alpha_al * rho_al / rho_l;
          //   double y_bl = alpha_bl * rho_bl / rho_l;
          //   double energy_l = EOS->compute_energy(alpha_al, rho_al, eps_al, rho_bl, eps_bl, vx_l, vy_l);
          //   arr(i,j,k,6) = rho_l * energy_l;

          //   // rho * v_y
          //   arr(i,j,k,7) = rho_l * vy_l;
          // }
          // else
          // {
          ...
          //   arr(i,j,k,3) = alpha_ar * rho_ar * eps_ar; // alpha_a * rho_a * eps_xa
          //   arr(i,j,k,4) = alpha_br * rho_br * eps_br; // alpha_b * rho_b * eps_xb

          //   // rho * v_x
          //   double rho_r = alpha_ar * rho_ar + alpha_br * rho_br;
          //   arr(i,j,k,5) = rho_r * vx_r;

          //   // rho * energy
          //   double y_ar = alpha_ar * rho_ar / rho_r;
          //   double y_br = alpha_br * rho_br / rho_r;
          //   double energy_r = EOS->compute_energy(alpha_ar, rho_ar, eps_ar, rho_br, eps_br, vx_r, vy_r);
          //   arr(i,j,k,6) = rho_r * energy_r; 

          //   // rho * v_y
          //   arr(i,j,k,7) = rho_r * vy_r;
          // }

          // // Initial condition for test 4
          // if(x>(12-sqrt(M_PI))/10)
          // {
          //   arr(i,j,k,0) = alpha_ar; // alpha_a
          //   arr(i,j,k,1) = alpha_ar * rho_ar; // alpha_a * rho_a
          //   arr(i,j,k,2) = alpha_br * rho_br; // alpha_b * rho_b
          ...
          // }
          // else if(x<(12-sqrt(M_PI))/10)
          // {
          //   if((pow((x-1.2),2) + pow((y-0.5),2)) < 0.6*0.6)
          //   {
          //   arr(i,j,k,0) = alpha_ar; // alpha_a
          //   arr(i,j,k,1) = alpha_ar * rho_ar; // alpha_a * rho_a
          //   arr(i,j,k,2) = alpha_br * rho_br; // alpha_b * rho_b
          ...
          //   arr(i,j,k,7) = rho_r * vy_r;
          //   }
          //   else if((pow((x-1.2),2) + pow((y-0.5),2)) > 0.6*0.6)
          //   {
          //   arr(i,j,k,0) = alpha_al; // alpha_a
          //   arr(i,j,k,1) = alpha_al * rho_al; // alpha_a * rho_a
          //   arr(i,j,k,2) = alpha_bl * rho_bl; // alpha_b * rho_b
          ...
          // Initial condition for test 5
          if((pow((x),2) + pow(y,2)) > pow(0.0001,2))
          {
            arr(i,j,k,0) = alpha_ar; // alpha_a
            arr(i,j,k,1) = alpha_ar * rho_ar; // alpha_a * rho_a
            arr(i,j,k,2) = alpha_br * rho_br; // alpha_b * rho_b
          ...
          else if((pow((x),2) + pow(y,2)) < pow(0.0001,2))
          {
            arr(i,j,k,0) = alpha_al; // alpha_a
            arr(i,j,k,1) = alpha_al * rho_al; // alpha_a * rho_a
            arr(i,j,k,2) = alpha_bl * rho_bl; // alpha_b * rho_b
          ...
          // // Initial condition for test 7
          // if(x>0.225) // post shocked air
          // {
          //   arr(i,j,k,0) = 0.999999; // alpha_a
          //   arr(i,j,k,1) = 0.999999 * 1.3764; // alpha_a * rho_a
          //   arr(i,j,k,2) = 1e-6 * 0.138 ; // alpha_b * rho_b (helium)

          //   double eps_a2 = (1.5698 + gamma_a * pinf_a) / ((gamma_a - 1) * 1.3764);
          //   arr(i,j,k,3) = 0.999999 * 1.3764 * eps_a2; // alpha_a * rho_a * eps_xa

          //   double eps_b2 = (1.0 + gamma_b * pinf_b) / ((gamma_b - 1) * 0.138);
          //   arr(i,j,k,4) = 1e-6 * 0.138 * eps_b2; // alpha_b * rho_b * eps_xb

          //   double rho2 = 0.999999 * 1.3764 + 1e-6 * 0.138;

          //   arr(i,j,k,5) = rho2 * - 0.394; // rho * v_x

          //   double energy_r = EOS->compute_energy(1.0, 1.3764, eps_a2, 0.138, eps_b2, - 0.394, 0.0); 
          //   arr(i,j,k,6) = rho2 * energy_r; // rho * energy

          //   arr(i,j,k,7) = 0.0; // rho * v_y

          // }
          // else if(x<0.225)
          // {
          //   if((pow((x-0.18),2) + pow((y-0.0445),2)) < 0.02*0.02) // anything inside circle is helium bubble
          //   {
          //   arr(i,j,k,0) = alpha_ar; // alpha_a
          //   arr(i,j,k,1) = alpha_ar * rho_ar; // alpha_a * rho_a
          //   arr(i,j,k,2) = alpha_br * rho_br; // alpha_b * rho_b
          ...
          //   arr(i,j,k,7) = rho_r * vy_r;
          //   }
          //   else if((pow((x-0.18),2) + pow((y-0.0445),2)) > 0.02*0.02) // anything outside circle is post-shocked air
          //   {
          //   arr(i,j,k,0) = alpha_al; // alpha_a
          //   arr(i,j,k,1) = alpha_al * rho_al; // alpha_a * rho_a
          //   arr(i,j,k,2) = alpha_bl * rho_bl; // alpha_b * rho_b
                      ...
                      int  ncycle)
{
  ...
  // 2020H: defining boundary conditions
  // //Boundary condition for test 3
  ...
  // for(int l=0; l<NUM_STATE; ++l)//++i and i++ are the same in a for loop
  // {
  //     if(l==5 || l==8 || l==12) // BC for v_x 
  //     {
  //       ArrayofBC[l].setLo(0, BCType::reflect_odd);
  //       ArrayofBC[l].setHi(0, BCType::foextrap);

  //       ArrayofBC[l].setLo(1, BCType::reflect_even);
  //       ArrayofBC[l].setHi(1, BCType::reflect_even);
  //     }

  //     else if(l==7 || l==9 || l==13) // BC for v_y 
  //     {
  //       ArrayofBC[l].setLo(0, BCType::reflect_even);
  //       ArrayofBC[l].setHi(0, BCType::foextrap);

  //       ArrayofBC[l].setLo(1, BCType::reflect_odd);
  //       ArrayofBC[l].setHi(1, BCType::reflect_odd);
  //     }

  //     else // BC for scalar variables 
  //     {
  //       ArrayofBC[l].setLo(0, BCType::reflect_even);
  //       ArrayofBC[l].setHi(0, BCType::foextrap);
  ...
  //     if(l==5) // BC for v_x 
  //     {
  //       ArrayofBC[l].setLo(0, BCType::foextrap);
  //       ArrayofBC[l].setHi(0, BCType::reflect_odd);
  //       // ArrayofBC[l].setHi(0, BCType::ext_dir);

  //       ArrayofBC[l].setLo(1, BCType::foextrap);
  //       ArrayofBC[l].setHi(1, BCType::foextrap);
  //     }

  //     else // BC for scalar variables 
  //     {
  //       ArrayofBC[l].setLo(0, BCType::foextrap);
  //       ArrayofBC[l].setHi(0, BCType::reflect_even);
  ...
  // Boundary condition for test 5 vy
  for(int l=0; l<NUM_STATE; ++l)//++i and i++ are the same in a for loop
  {
      if(l==5) // BC for v_x
      {
  ...
  // 2020H: implement the vibrating BC here (Cartesian code)
  //   for(int k = lo.z; k <= hi.z; k++)
  //   {
  //     for(int j = lo.y-2; j <= hi.y+2; j++)
  //     {
  //       if (hi.x >= geom.Domain().bigEnd(0))
  //       {
  //       arr(hi.x+1,j,k,5) = (arr(hi.x+1,j,k,1) + arr(hi.x+1,j,k,2)) * ( 2 * M_PI * 4e6 * 5e-6 * cos(2 * M_PI * 4e6 * time) * (0.5 + 0.5 * cos(M_PI * 2e-3))) + arr(hi.x+1,j,k,5);
  //       arr(hi.x+2,j,k,5) = (arr(hi.x+2,j,k,1) + arr(hi.x+2,j,k,2)) * ( 2 * M_PI * 4e6 * 5e-6 * cos(2 * M_PI * 4e6 * time) * (0.5 + 0.5 * cos(M_PI * 2e-3))) + arr(hi.x+2,j,k,5);
  //       }
  //     }
  //   }
  // }

  // implement the vibrating BC here (Cylindrical code)
  if (lo.y <= geom.Domain().smallEnd(1))
  {
    for(int k = lo.z; k <= hi.z; k++)
    {
        for(int i = lo.x-2; i <= hi.x+2; i++)
        {
          double x = 0.0 + (i+0.5)*dx[0];
          arr(i,lo.y-1,k,7) = (arr(i,lo.y-1,k,1) + arr(i,lo.y-1,k,2)) * ( -2 * M_PI * 2e6 * 5e-6 * cos(2 * M_PI * 2e6 * time) * (0.5 + 0.5 * cos(M_PI * x / 1e-4))) + arr(i,lo.y-1,k,7);
          arr(i,lo.y-2,k,7) = (arr(i,lo.y-2,k,1) + arr(i,lo.y-2,k,2)) * ( -2 * M_PI * 2e6 * 5e-6 * cos(2 * M_PI * 2e6 * time) * (0.5 + 0.5 * cos(M_PI * x / 1e-4))) + arr(i,lo.y-2,k,7);
        }
      }
    }
  }
    ...
    for(int k = lo.z; k <= hi.z; k++)
    {
      for(int j = lo.y; j <= hi.y+2; j++)
      {
        for(int i = lo.x; i <= hi.x+2; i++)
        {
          ...
          double nVar = NUM_STATE - 7;
          for(int l=0; l<nVar-2; l++) // skip energy, vy and only pass in 6 variables into second order extension function
          {
          ...
          // push vy up one column 
          ul_n[nVar-2] = arr(i-2,j-1,k,nVar-1);
          ul[nVar-2] = arr(i-1,j-1,k,nVar-1);
          ul_p[nVar-2] = arr(i,j-1,k,nVar-1);

          // push energy down one column 
          ul_n[nVar-1] = arr(i-2,j-1,k,nVar-2);
          ul[nVar-1] = arr(i-1,j-1,k,nVar-2);
          ul_p[nVar-1] = arr(i,j-1,k,nVar-2);

          // reconstructing i-1 cell
          Arrayofdouble primiL, primiR;
          NM->secondorder_extension(ul_n, ul, ul_p, gamma_a, gamma_b, pinf_a, pinf_b, primiL, primiR, nVar, dx[0], sl_const);
    ...
    for(int k = lo.z; k <= hi.z; k++)
    {
      for(int j = lo.y; j <= hi.y+2; j++)
      {
        for(int i = lo.x; i <= hi.x+2; i++)
        {
          ...
          // push vy up one column 
          yul_n[nVar-2] = arr(i-1,j-2,k,nVar-1);
          yul[nVar-2] = arr(i-1,j-1,k,nVar-1);
          yul_p[nVar-2] = arr(i-1,j,k,nVar-1);

          // push energy down one column 
          yul_n[nVar-1] = arr(i-1,j-2,k,nVar-2);
          yul[nVar-1] = arr(i-1,j-1,k,nVar-2);
          yul_p[nVar-1] = arr(i-1,j,k,nVar-2);

          // reconstructing j-1 cell
          Arrayofdouble yprimiL, yprimiR;
          NM->secondorder_extension(yul_n, yul, yul_p, gamma_a, gamma_b, pinf_a, pinf_b, yprimiL, yprimiR, nVar, dx[1], sl_const);

          for(int m=0; m<nVar; m++) // storing reconstructed values as the same index as cell as number of cells equal to number of left and right wall
          {
          arr_yrecon_l(i-1,j-1,k,m) = yprimiL[m]; // left wall of cell (i-1, j-1)
          arr_yrecon_r(i-1,j-1,k,m) = yprimiR[m]; // right wall of cell (i-1, j-1)
          }
      ...
      for(int k = lo.z; k <= hi.z; k++)
      {
        for(int j = lo.y; j <= hi.y+2; j++) // previous dimensional loop already closed, still 1 for half time step evolution
        {
          for(int i = lo.x; i <= hi.x+2; i++) // previous dimensional loop already closed, still 1 for half time step evolution
          {
            ...
            double energyfluxL, energyfluxR, yenergyfluxL, yenergyfluxR;
            
            for(int l=0; l<nVar-2; l++)
            {
              ui[l] = arr(i-1,j-1,k,l); // use cell centre of cell -1 to hi+1
            }

            ui[nVar-2] = arr(i-1,j-1,k,nVar-1); // use cell centre of cell -1 to hi+1
            ui[nVar-1] = arr(i-1,j-1,k,nVar-2); // use cell centre of cell -1 to hi+1

            primi = EOS->secondu_to_prim(ui, gamma_a, pinf_a, gamma_b, pinf_b);
            ...
            energyfluxL = EOS->energyflux(primiL, gamma_a, pinf_a, gamma_b, pinf_b);
            energyfluxR = EOS->energyflux(primiR, gamma_a, pinf_a, gamma_b, pinf_b);
            yenergyfluxL = EOS->yenergyflux(yprimiL, gamma_a, pinf_a, gamma_b, pinf_b);
            yenergyfluxR = EOS->yenergyflux(yprimiR, gamma_a, pinf_a, gamma_b, pinf_b);

            // half time step evolution for left, right reconstructed variables (all variables except energy)
            NM->halftimestep_update(primi, primiL, primiR, yprimi, yprimiL, yprimiR, dt, dx[0], dx[1], uiL_nplushalf, uiR_nplushalf, gamma_a, gamma_b, pinf_a, pinf_b);
            ...
            // second order energy
            arr_xrecon_lht(i-1,j-1,k,nVar-2) = primiL[nVar-1] + 0.5 * dt * dx[0] * (energyfluxL - energyfluxR) + 0.5 * dt * dx[1] * (yenergyfluxL - yenergyfluxR);
            arr_xrecon_rht(i-1,j-1,k,nVar-2) = primiR[nVar-1] + 0.5 * dt * dx[0] * (energyfluxL - energyfluxR) + 0.5 * dt * dx[1] * (yenergyfluxL - yenergyfluxR);
            
            // second order vy
            arr_xrecon_lht(i-1,j-1,k,nVar-1) = uiL_nplushalf[nVar-2];
            arr_xrecon_rht(i-1,j-1,k,nVar-1) = uiR_nplushalf[nVar-2];

            // half time step evolution for top, bottom reconstructed variables (all variables except energy)
            NM->yhalftimestep_update(primi, primiL, primiR, yprimi, yprimiL, yprimiR, dt, dx[0], dx[1], yuiL_nplushalf, yuiR_nplushalf, gamma_a, gamma_b, pinf_a, pinf_b);
            ...
            // second order energy
            arr_yrecon_lht(i-1,j-1,k,nVar-2) = yprimiL[nVar-1] + 0.5 * dt * dx[0] * (energyfluxL - energyfluxR) + 0.5 * dt * dx[1] * (yenergyfluxL - yenergyfluxR);
            arr_yrecon_rht(i-1,j-1,k,nVar-2) = yprimiR[nVar-1] + 0.5 * dt * dx[0] * (energyfluxL - energyfluxR) + 0.5 * dt * dx[1] * (yenergyfluxL - yenergyfluxR);

            // second order vy
            arr_yrecon_lht(i-1,j-1,k,nVar-1) = yuiL_nplushalf[nVar-2];
            arr_yrecon_rht(i-1,j-1,k,nVar-1) = yuiR_nplushalf[nVar-2];
    ...
    for (MFIter mfi(Sborder, true); mfi.isValid(); ++mfi)
    {
      ...
      for(int k = lo.z; k <= hi.z+kOffset; k++)
      {
        for(int j = lo.y; j <= hi.y+jOffset; j++)
        {
          ...
          {
            // x flux computation
            NM->compute_HLLCflux(arr, arr_xrecon_lht, arr_xrecon_rht, flux, wavespeed, NUM_STATE-7, gamma_a, pinf_a, gamma_b, pinf_b, i, j ,k, dx[d], dt);
            
            for(int l = 0; l<NUM_STATE-7; l++)
            {
              fluxArr(i,j,k,l) = flux[l];
            }

            fluxArr(i,j,k,8) = wavespeed[2]; // intermediate wave speed 
            fluxArr(i,j,k,12) = wavespeed[3];          
          }
        }
      }
      } 
      ...
      for(int k = lo.z; k <= hi.z+kOffset; k++)
      {
        for(int j = lo.y; j <= hi.y+jOffset; j++)
        {
          for(int i = lo.x; i <= hi.x+iOffset; i++)
          {
            ...
            NM->ycompute_HLLCflux(arr, arr_yrecon_lht, arr_yrecon_rht, yflux, ywavespeed, NUM_STATE-7, gamma_a, pinf_a, gamma_b, pinf_b, i, j ,k, dx[d], dt);
            ...
            yfluxArr(i,j,k,9) = ywavespeed[2]; // intermediate wave speed   
            yfluxArr(i,j,k,13) = ywavespeed[3];
          }
        }
      }
      }
  ...
  // 2020H: cylindrical coordinate transformation, Godunov update, pressure relaxation
  // Loop over all the patches at this level
  #ifdef AMREX_USE_OMP
  #pragma omp parallel
  #endif
  ...
  //source term update for cylindrical coordinate
  for(int k = lo.z; k <= hi.z; k++)
  {
    for(int j = lo.y-2; j <= hi.y+2; j++)
    {
      for(int i = lo.x-2; i <= hi.x+2; i++)
      {
        ...
        double eps_a = arr(i,j,k,3) / (alpha_a * rho_a);
        double eps_b = arr(i,j,k,4) / (alpha_b * rho_b);
        double p_a = (gamma_a - 1) * rho_a * eps_a - gamma_a * pinf_a;
        double p_b = (gamma_b - 1) * rho_b * eps_b - gamma_b * pinf_b;

        double S = fluxArr(i,j,k,12);
        double S_p = fluxArr(i+1,j,k,12);
        double yS = yfluxArr(i,j,k,13);
        double yS_p = yfluxArr(i,j+1,k,13);

        double rho = arr(i,j,k,1) + arr(i,j,k,2);
        double v_x = arr(i,j,k,5) / rho;
        double v_y = arr(i,j,k,7) / rho;
        double energy = arr(i,j,k,6) / rho;
        // double pressure = alpha_a * p_a + alpha_b * p_b;
        double pressure = EOS->compute_pressure(alpha_a, rho_a, gamma_a, pinf_a, rho_b, gamma_b, pinf_b, energy, v_x, v_y); // mixture pressure

        // arr(i,j,k,0) = arr(i,j,k,0) - (dt / x) * (alpha_a * v_x); //alpha_a * rho_a
        arr(i,j,k,1) = arr(i,j,k,1) - (dt / x) * (alpha_a * rho_a * v_x); //alpha_a * rho_a
        arr(i,j,k,2) = arr(i,j,k,2) - (dt / x) * (alpha_b * rho_b * v_x); //alpha_b * rho_b
        arr(i,j,k,3) = arr(i,j,k,3) - (dt / x) * (alpha_a * rho_a * eps_a * v_x); // internal energy a update
        arr(i,j,k,4) = arr(i,j,k,4) - (dt / x) * (alpha_b * rho_b * eps_b * v_x); // internal energy b update
        arr(i,j,k,5) = arr(i,j,k,5) - (dt / x) * (rho * v_x * v_x); //rho * v_x
        arr(i,j,k,6) = arr(i,j,k,6) - (dt / x) * ((rho * energy + pressure) * v_x); //rho * energy
        arr(i,j,k,7) = arr(i,j,k,7) - (dt / x) * (rho * v_x * v_y); //rho * v_y
      }
    }
  }
      ...
      for(int k = lo.z; k <= hi.z; k++)
      {
        for(int j = lo.y; j <= hi.y; j++)
        {
          for(int i = lo.x; i <= hi.x; i++)
          {
            ...
            double S_star = fluxArr(i,j,k,8);
            double S_star_p = fluxArr(i+1,j,k,8);
            double yS_star = yfluxArr(i,j,k,9);
            double yS_star_p = yfluxArr(i,j+1,k,9);
            ...
            for(int l = 0; l<NUM_STATE-7; l++)
            {
              if(l == 0)
              {
                arr(i,j,k,l) = arr(i,j,k,l) - (dt / dx[0]) * (fluxArr(i+1,j,k,l)*S_star_p - fluxArr(i,j,k,l)*S_star - alpha_a * (S_star_p - S_star)) - (dt / dx[1]) * (yfluxArr(i,j+1,k,0)*yS_star_p - yfluxArr(i,j,k,0)*yS_star - alpha_a * (yS_star_p - yS_star)); // volume fraction a update
              }
              else if(l == 3)
              {
                arr(i,j,k,3) = arr(i,j,k,3) - (dt / dx[0]) * (fluxArr(i+1,j,k,3)*S_p - fluxArr(i,j,k,3)*S + alpha_a * p_a * (S_p - S)) - (dt / dx[1]) * (yfluxArr(i,j+1,k,3)*yS_p - yfluxArr(i,j,k,3)*yS + alpha_a * p_a * (yS_p - yS)); // internal energy a update
              }
              else if(l == 4)
              {
                arr(i,j,k,4) = arr(i,j,k,4) - (dt / dx[0]) * (fluxArr(i+1,j,k,4)*S_p - fluxArr(i,j,k,4)*S + alpha_b * p_b * (S_p - S)) - (dt / dx[1]) * (yfluxArr(i,j+1,k,4)*yS_p - yfluxArr(i,j,k,4)*yS + alpha_b * p_b * (yS_p - yS)); // internal energy b update
              }
              else
              {
            ...
            arr(i,j,k,10) = arr(i,j,k,1) + arr(i,j,k,2); // mixture density for visit
            arr(i,j,k,11) = arr(i,j,k,7)/arr(i,j,k,10); // vx for visit

            double alpha_a2 = arr(i,j,k,0);
            double alpha_b2 = 1.0 - arr(i,j,k,0);

            double rho_a2 = arr(i,j,k,1) / alpha_a2;
            double rho_b2 = arr(i,j,k,2) / alpha_b2;

            double eps_a2 = arr(i,j,k,3) / (alpha_a2 * rho_a2);
            double eps_b2 = arr(i,j,k,4) / (alpha_b2 * rho_b2);

            double p_a2 = EOS->epstopressure(rho_a2, eps_a2, pinf_a, gamma_a);
            double p_b2 = EOS->epstopressure(rho_b2, eps_b2, pinf_b, gamma_b);

            arr(i,j,k,14) = alpha_a2 * p_a2 + alpha_b2 * p_b2;
          }
        }
      }
      ...
      for(int k = lo.z; k <= hi.z; k++)
      {
        for(int j = lo.y; j <= hi.y; j++)
        {
          for(int i = lo.x; i <= hi.x; i++)
          {
            Arrayofdouble relaxed_terms;
            NM->pressure_relaxation(arr, i, j ,k, gamma_a, pinf_a, gamma_b, pinf_b, relaxed_terms);

              arr(i,j,k,0) = relaxed_terms[0];
              arr(i,j,k,3) = relaxed_terms[1];
              arr(i,j,k,4) = relaxed_terms[2];
          }
        }
      }
    }// close MFIter loop here
  ...
  if (lo.y <= geom.Domain().smallEnd(1))
  {
    //2020H: implement the vibrating BC here (Cylindrical code)
    for(int k = lo.z; k <= hi.z; k++)
    {
      for(int i = lo.x-2; i <= hi.x+2; i++)
      {
    ...
    for(int k = lo.z; k <= hi.z; k++)
    {
      for(int j = lo.y; j <= hi.y+2; j++)
      {
        for(int i = lo.x; i <= hi.x+2; i++)
        {
          ...
          double rho_a = arr(i,j,k,1) / alpha_a;
          double rho_b = arr(i,j,k,2) / alpha_b;
          double rho = alpha_a * rho_a + alpha_b * rho_b;
          double vx = arr(i,j,k,5) / rho;
          double vy = arr(i,j,k,7) / rho;
          double vm = sqrt(vx*vx + vy*vy);

          // computing sound speed cs
          double eps_a = arr(i,j,k,3) / (alpha_a * rho_a);
          double eps_b = arr(i,j,k,4) / (alpha_b * rho_b);
          double p_a = EOS->epstopressure(rho_a, eps_a, pinf_a, gamma_a);
          double p_b = EOS->epstopressure(rho_b, eps_b, pinf_b, gamma_b);

          double c_a = sqrt(gamma_a * (p_a + pinf_a) / rho_a);
          double c_b = sqrt(gamma_b * (p_b + pinf_b) / rho_b);

          double y_a = alpha_a * rho_a / rho;
          double y_b = alpha_b * rho_b / rho;
          
          double cs = sqrt(y_a*c_a*c_a + y_b*c_b*c_b); // frozen mixture sound speed
