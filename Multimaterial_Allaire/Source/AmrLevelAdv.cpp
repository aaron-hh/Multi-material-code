...
int      AmrLevelAdv::NUM_STATE       = 12;  // One variable in the state
    ...
    for(int k = lo.z; k <= hi.z; k++)
    {
      for(int j = lo.y; j <= hi.y; j++)
      {
        for(int i = lo.x; i <= hi.x; i++)
        {
          ...
          // double pressure = EOS->compute_pressure(arr(i,j,k,0), arr(i,j,k,1), arr(i,j,k,2), energy, vx, vy, gamma_a, pinf_a, gamma_b, pinf_b);

          // double i_energy = arr(i,j,k,11);
          ...
          // if(y==1.055)
          // {
          // output<<x<<" "<<y<<" "<<alpha_a<<" "<<density<<" "<<vx<<" "<<pressure<<" "<<energy<<" "<<vy<<" "<<i_energy<<std::endl;
          // }
        }
        // output<<std::endl;
      }
...
// 2020H: defining boundary condition for 1D test 1,2,3
...
//   BCRec bc(lo_bc, hi_bc);
  ...
  for (int i = 0; i < amrex::SpaceDim; ++i) 
  {
    ...
    {
      lo_bc2[i] = hi_bc2[i] = BCType::reflect_even;
    }
  }
  // Object for storing all the boundary conditions
  ...
  desc_lst.setComponent(Phi_Type, 3, "rho_vx", bc2, bndryfunc2);
  desc_lst.setComponent(Phi_Type, 4, "rho_energy", bc, bndryfunc2);
  desc_lst.setComponent(Phi_Type, 5, "rho_vy", bc3, bndryfunc2);
  ...
  desc_lst.setComponent(Phi_Type, 11, "Internal Energy", bc, bndryfunc2);
}
    ...
    for(int k = lo.z; k <= hi.z; k++)
    {
      ...
      for(int j = lo.y; j <= hi.y; j++)
      {
        ...
        for(int i = lo.x; i <= hi.x; i++)
        {
          ...
          // // Initial condition for test 1, 2, 3
          ...
          //   // rho * energy
          //   double energy_l = EOS->compute_energy(arr(i,j,k,0), arr(i,j,k,1), arr(i,j,k,2), p_l, vx_l, vy_l, gamma_a, pinf_a, gamma_b, pinf_b);
          ...
          //   // rho * energy
          //   double energy_r = EOS->compute_energy(arr(i,j,k,0), arr(i,j,k,1), arr(i,j,k,2), p_r, vx_r, vy_r, gamma_a, pinf_a, gamma_b, pinf_b);
          ...
          if(x>0.225) // post shocked air
          {
            ...
            arr(i,j,k,3) = rho2 * - 0.394; // rho * v_x

            double energy_r = EOS->compute_energy(arr(i,j,k,0), arr(i,j,k,1), arr(i,j,k,2), 1.5698, -0.394, 0, gamma_a, pinf_a, gamma_b, pinf_b);

            arr(i,j,k,4) = rho2 * energy_r; // rho * energy

            arr(i,j,k,5) = 0.0; // rho * v_y
          }

          else if(x<0.225)
          {
            if((pow((x-0.175),2) + pow(y,2)) < 0.025*0.025) // anything inside circle is helium bubble
            {
            arr(i,j,k,0) = alpha_ar; // alpha_a
            arr(i,j,k,1) = alpha_ar * rho_ar; // alpha_a * rho_a
            arr(i,j,k,2) = alpha_br * rho_br; // alpha_b * rho_b
            ...
            else if((pow((x-0.175),2) + pow(y,2)) > 0.025*0.025) // anything outside circle is post-shocked air
            {
            arr(i,j,k,0) = alpha_al; // alpha_a
            arr(i,j,k,1) = alpha_al * rho_al; // alpha_a * rho_a
            arr(i,j,k,2) = alpha_bl * rho_bl; // alpha_b * rho_b
      ...
      for(int k = lo.z; k <= hi.z; k++)
      {
        for(int j = lo.y; j <= hi.y+2; j++) // previous dimensional loop already closed, still 1 for half time step evolution
        {
          for(int i = lo.x; i <= hi.x+2; i++) // previous dimensional loop already closed, still 1 for half time step evolution
          {
            ...
            primi = EOS->u_to_prim(ui, gamma_a, pinf_a, gamma_b, pinf_b);
      ...
      for(int k = lo.z; k <= hi.z+kOffset; k++)
      {
        for(int j = lo.y; j <= hi.y+jOffset; j++)
        {
          for(int i = lo.x; i <= hi.x+iOffset; i++)
          {
            ...
            // x flux computation
            NM->compute_HLLCflux(arr, arr_xrecon_lht, arr_xrecon_rht, flux, wavespeed, nVar, gamma_a, pinf_a, gamma_b, pinf_b, i, j ,k, dx[d], dt);
      ...
      for(int k = lo.z; k <= hi.z+kOffset; k++)
      {
        for(int j = lo.y; j <= hi.y+jOffset; j++)
        {
          for(int i = lo.x; i <= hi.x+iOffset; i++)
          {
            ...
            NM->ycompute_HLLCflux(arr, arr_yrecon_lht, arr_yrecon_rht, yflux, ywavespeed, nVar, gamma_a, pinf_a, gamma_b, pinf_b, i, j ,k, dx[d], dt);
      ...
      for(int k = lo.z; k <= hi.z; k++)
      {
        for(int j = lo.y; j <= hi.y; j++)
        {
          for(int i = lo.x; i <= hi.x; i++)
          {
            ...
            arr(i,j,k,10) = EOS->compute_pressure(alpha_a2, arr(i,j,k,1), arr(i,j,k,2), arr(i,j,k,4)/rho2, arr(i,j,k,3)/rho2, arr(i,j,k,5)/rho2, gamma_a, pinf_a, gamma_b, pinf_b);

            double vy = arr(i,j,k,5) / rho2;
            arr(i,j,k,11) = (arr(i,j,k,4) - 0.5 * rho2 * (arr(i,j,k,9) * arr(i,j,k,9) + vy * vy)) / rho2; // internal energy for visit
          }
        }
      }
    }// close MFIter loop here
    ...
    for(int k = lo.z; k <= hi.z; k++)
    {
      for(int j = lo.y; j <= hi.y; j++)
      {
        for(int i = lo.x; i <= hi.x; i++)
        {
          ...
          double p = EOS->compute_pressure(arr(i,j,k,0), arr(i,j,k,1), arr(i,j,k,2), arr(i,j,k,4)/rho, arr(i,j,k,3)/rho, arr(i,j,k,5)/rho, gamma_a, pinf_a, gamma_b, pinf_b);
          double cs = EOS->compute_soundspeed(arr(i,j,k,0), arr(i,j,k,1), arr(i,j,k,2), p, gamma_a, pinf_a, gamma_b, pinf_b);
