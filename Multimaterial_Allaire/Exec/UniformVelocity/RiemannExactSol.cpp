...
//Newton Raphson
do // similar to while loop but do-while is guaranteed to execute once
{
...
for(int i=0; i<nCells; i++)
{
  ...
  // convert pressure to specific internal energy
  double energy_i = u[i][2] / (u[i][0] * (gamma - 1));

  output<<x<<" "<<u[i][0]<<" "<<u[i][1]<<" "<<u[i][2]<<" "<<energy_i<<std::endl;
}
}
